...
function ignore_alias_fetch($al, &$is_mod)
{
	...
	if (isset($_POST['add_login']) && is_string($_POST['add_login'])) {
		if (!($ignore_id = ignore_alias_fetch($_POST['add_login'], $is_mod))) {
			error_dialog('De gebruikers is niet gevonden', 'De gebruiker die u aan uw negeerlijst probeerde toe te voegen is niet gevonden.');
		}
		if ($is_mod) {
...
<tr class="RowStyleA">
	<td class="GenText nw Smalltext">
		Geef de gebruikersnaam van de gebruiker die u wilt toevoegen op.<?php echo (($FUD_OPT_1 & (8388608|4194304)) ? '<br />Of gebruik de optie <a href="javascript://" onclick="javascript: window_open(&#39;'.$GLOBALS['WWW_ROOT'].'index.php?t=pmuserloc&amp;'._rsid.'&amp;js_redr=buddy_add.add_login&amp;overwrite=1&#39;, &#39;user_list&#39;, 400,250);">Gebruiker zoeken</a> om een gebruiker te vinden.' : ''); ?><br /><br />
