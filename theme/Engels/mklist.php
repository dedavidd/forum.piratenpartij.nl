...
function addOption()
{
	...
	dl.appendChild(document.createTextNode('Verwijderen'));
...
<tr>
	...
	<td>
		<select name="tp" onchange="changeListType(this.options[this.selectedIndex].value);">
			<option value="1">Numeriek</option>
			<option value="a">Alfanumeriek</option>
			<option value="square"<?php echo ($def_list_type == 'square' ? ' selected="selected"' : ''); ?>>Vierkant</option>
			<option value="disc">Schijf</option>
...
<tr>
	...
	<td class="nw">
		...
		<input tabindex="2" type="button" class="button" name="btn_submit" onclick="addOption();" value="Item toevoegen" />
	</td>
</tr>
<tr>
...
<tr>
	<td colspan="2" class="ar">
		<input type="button" class="button" name="go" value="Toepassen" onclick="updatePostForm();" />
		<input type="button" class="button" name="close" value="Sluiten" onclick="window.close();" />
	</td>
</tr>
</table>
</form>
