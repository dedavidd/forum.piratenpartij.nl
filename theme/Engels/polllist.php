...
* $Id: polllist.php.t 4994 2010-09-02 17:33:29Z naudefj $
*
...
function tmpl_create_pager($start, $count, $total, $arg, $suf='', $append=1, $js_pager=0, $no_append=0)
{
	...
	if (!empty($_GET['goto'])) {
		...
		header('Location: '.$GLOBALS['WWW_ROOT'].'index.php?t='. d_thread_view .'&goto='. $mid .'&pl_view='. $pl_view .'&'._rsidl .'#msg_'. $mid);
		return;
	}

	ses_update_status($usr->sid, '<a href="index.php?t=polllist">Beschikbare stemmingen bekijken</a>');
...
if (_uid) {
	...
	while ($obj = db_rowobj($c)) {
		$view_res_lnk = $obj->total_votes ? '<b>::</b> <a href="index.php?t=polllist&amp;goto='.$obj->id.'&amp;vote=1&amp;'._rsid.'#msg_'.$obj->id.'">Resultaten bekijken</a>' : '';
		if (!$obj->total_votes) {
			$obj->total_votes = '0';
		}
		if ($obj->owner && (!($obj->users_opt & 32768) || $is_a) && $FUD_OPT_2 & 32) {
			$online_indicator = $obj->last_visit > __request_timestamp__ ? '<img src="theme/default/images/online'.img_ext.'" title="'.$obj->login.' is op dit moment aanwezig" alt="'.$obj->login.' is op dit moment aanwezig" />&nbsp;' : '<img src="theme/default/images/offline'.img_ext.'" title="'.$obj->login.' is op dit moment afwezig" alt="'.$obj->login.' is op dit moment afwezig" />&nbsp;';
		} else {
			$online_indicator = '';
		}
	...
	<td class="nw ac"> <b>::</b> <a href="index.php?t=polllist&amp;goto='.$obj->id.'&amp;'._rsid.'#msg_'.$obj->id.'">Stemmen</a> '.$view_res_lnk.' <b>::</b></td>
</tr>';
	}
	unset($c);
	...
	if ($ttl > $POLLS_PER_PAGE) {
		...
		$poll_entries = '<tr>
	<td colspan="5" class="ac">Er zijn geen stemmingen beschikbaar.</td>
</tr>';
	}
...
<tr>
	<th>Stemmingsnaam</th>
	<th class="nw"><a class="thLnk" href="index.php?t=polllist&amp;start=<?php echo $start; ?>&amp;oby=<?php echo $oby_rev_val; ?>&amp;<?php echo _rsid; ?>">Aangemaakt op</a></th>
	<th class="nw">aangemaakt door</th>
	<th class="nw ac">Totaal aantal stemmen</th>
