...
function draw_tmpl_perm_table($perm, $perms, $names)
{
	...
	foreach ($perms as $k => $v) {
		$str .= ($perm & $v[0]) ? '<td title="'.$names[$k].'" class="permYES">Ja</td>' : '<td title="'.$names[$k].'" class="permNO">Nee</td>';
	}
	return $str;
}
...
<tr>
	<th colspan="3">Groep bewerken</th>
</tr>
<tr class="RowStyleC">
	<td class="nw fb">Groep:</td>
	<td><select name="group_id">'.tmpl_draw_select_opt(rtrim($vl), rtrim($kl), $group_id).'</select></td>
	<td class="ar"><input type="submit" class="button" name="btn_groupswitch" value="Groep bewerken" /></td>
</tr>
</table></div>'._hs.'</form>';
	}
...
/* Print number of unread private messages in User Control Panel. */
	...
	if (!$grp->forum_id) {
		$group_resources = '<b>Deze groep beheert de rechten voor de volgende forums:</b><br />';
		...
		$group_resources = '<b>Primaire groep voor forum:</b> '.$fname;
	}
	...
	if (isset($_POST['btn_submit'])) {
		...
		if (empty($_POST['edit'])) {
			...
			if (!($usr_id = q_singleval('SELECT id FROM fud30_users WHERE alias='. _esc(char_fix(htmlspecialchars($gr_member)))))) {
				$login_error = '<span class="ErrorText">Er is geen gebruiker met de gebruikersnaam "'.char_fix(htmlspecialchars($gr_member)).'"</span><br />';
			} else if (q_singleval('SELECT id FROM fud30_group_members WHERE group_id='. $group_id .' AND user_id='. $usr_id)) {
				$login_error = '<span class="ErrorText">Gebruiker "'.char_fix(htmlspecialchars($gr_member)).'" is al lid van deze groep.</span><br />';
			} else {
	...
	if (isset($_GET['edit']) && ($edit = (int)$_GET['edit'])) {
		...
		if ($mbr->user_id == 0) {
			$gr_member = '<span class="anon">Anoniem</span>';
		} else if ($mbr->user_id == '2147483647') {
			$gr_member = '<span class="reg">Alle geregistreerde gebruikers</span>';
		} else {
			$gr_member = $mbr->alias;
		}
...
'p_EDIT'=>'Bewerken',
'p_DEL'=>'Verwijderen',
'p_STICKY'=>'Sticky berichten',
'p_POLL'=>'Stemmingen aanmaken',
'p_FILE'=>'Bestanden toevoegen',
'p_VOTE'=>'Stemmen',
'p_RATE'=>'Onderwerpen/leden beoordelen',
'p_SPLIT'=>'Onderwerpen splitsen',
'p_LOCK'=>'Onderwerpen sluiten',
'p_MOVE'=>'Onderwerpen verplaatsen',
'p_SML'=>'Smiley&#39;s gebruiken',
'p_IMG'=>'Afbeeldingslabels gebruiken',
'p_SEARCH'=>'Kan zoeken');
...
<select name="'.$k.'" class="SmallText">
	...
	<option value="'.$v[0].'"'.$selyes.'>Ja</option>
</select>
</td>';
		} else {
...
<tr class="RowStyleB">'.$perm_select.'</tr>';
	...
	while ($obj = db_rowobj($r)) {
		...
		if ($obj->user_id == '0') {
			$member_name = '<span class="anon">Anoniem</span>';
			$group_members_list .= '<tr class="'.alt_var('mem_list_alt','RowStyleA','RowStyleB').'">
	<td class="nw">'.$member_name.'</td>
	'.$perm_table.'
	<td class="nw">[<a href="index.php?t=groupmgr&amp;'._rsid.'&amp;edit='.$obj->mmid.'&amp;group_id='.$obj->group_id.'">Bewerken</a>]</td>
</tr>';
		} else if ($obj->user_id == '2147483647')  {
			$member_name = '<span class="reg">Alle geregistreerde gebruikers</span>';
			$group_members_list .= '<tr class="'.alt_var('mem_list_alt','RowStyleA','RowStyleB').'">
	<td class="nw">'.$member_name.'</td>
	'.$perm_table.'
	<td class="nw">[<a href="index.php?t=groupmgr&amp;'._rsid.'&amp;edit='.$obj->mmid.'&amp;group_id='.$obj->group_id.'">Bewerken</a>]</td>
	...
	<td class="nw">'.$member_name.'</td>
	'.$perm_table.'
	<td class="nw">[<a href="index.php?t=groupmgr&amp;'._rsid.'&amp;edit='.$obj->mmid.'&amp;group_id='.$obj->group_id.'">Bewerken</a>] [<a href="index.php?t=groupmgr&amp;'._rsid.'&amp;del='.$obj->user_id.'&amp;group_id='.$obj->group_id.'&amp;SQ='.$GLOBALS['sq'].'">Verwijderen</a>]</td>
</tr>';
			}
		}
	}
...
<br />
<div class="ac">Bezig met bewerken van: <b><?php echo $grp->name; ?></b><br /><?php echo $group_resources; ?></div>
<br />
<form method="post" action="index.php?t=groupmgr" id="groupmgr">
<table cellspacing="1" cellpadding="2" class="ContentTable">
<?php echo ($edit ? '
<tr class="RowStyleA">
	<td class="nw fb">Lid</td>
...
<tr class="RowStyleA">
	<td class="nw fb">Lid</td>
	<td class="wa al">'.$login_error.'<input tabindex="1" type="text" name="gr_member" value="'.(isset($_POST['gr_member']) ? char_fix(htmlspecialchars($_POST['gr_member'])).'' : '' )  .'" />'.($FUD_OPT_1 & (8388608|4194304) ? '&nbsp;&nbsp;&nbsp;[ <a href="javascript://" onclick="javascript: window_open(\'index.php?t=pmuserloc&amp;'._rsid.'&amp;js_redr=groupmgr.gr_member&amp;overwrite=1\', \'user_list\',400,250);">Gebruiker zoeken</a> ]' : '' )  .'</td>
</tr>
'); ?>
<tr class="RowStyleB">
	<td colspan="2">
...
<tr>
	<td colspan="2" class="RowStyleC ar">
		<?php echo ($edit ? '<input type="submit" tabindex="3" class="button" name="btn_cancel" value="Annuleren" /> <input type="submit" tabindex="2" class="button" name="btn_submit" value="Lidmaatschap bijwerken" />' : '<input type="submit" tabindex="2" class="button" name="btn_submit" value="Lid toevoegen" />'); ?>
	</td>
</tr>
</table>
...
<tr>
	<th>Lid</th>
	<th colspan="<?php echo count($hdr); ?>">Rechten <span class="small">(ga met uw muis over het recht om het type te bekijken)</span></th>
