...
function forum_notify_del($user_id, $forum_id)
{
	...
	ses_update_status($usr->sid, 'Persoonlijke beheervenster');
...
<tr>
	...
	while ($r = db_rowarr($c)) {
		...
		$obj = new stdClass();
		$obj->login = $r[1];
		$uc_online = (!$r[3] && ($r[2] + $LOGEDIN_TIMEOUT * 60) > __request_timestamp__) ? '<img src="theme/default/images/online'.img_ext.'" alt="'.$obj->login.' is op dit moment aanwezig" title="'.$obj->login.' is op dit moment aanwezig" />' : '<img src="theme/default/images/offline'.img_ext.'" alt="'.$obj->login.' is op dit moment afwezig" title="'.$obj->login.' is op dit moment afwezig" />';
...
'.$uc_new_pms;
	...
	while ($r = db_rowobj($c)) {
		$uc_sub_forum .= '<tr>
	<td class="RowStyleA SmallText wa"><a href="index.php?t='.t_thread_view.'&amp;frm_id='.$r->id.'&amp;'._rsid.'" class="big">'.htmlspecialchars($r->cat_name).' &raquo; '.$r->name.'</a>'.($r->descr ? '<br />
'.$r->descr.'' : '' ) .'<br /><a href="index.php?t=post&amp;frm_id='.$r->id.'&amp;'._rsid.'">Nieuw onderwerp</a> || <a href="index.php?t=uc&amp;ufid='.$r->id.'&amp;'._rsid.'&amp;SQ='.$GLOBALS['sq'].'">Uitschrijven</a></td>
...
<br />
Door: '.($r->alias ? '<a href="index.php?t=usrinfo&amp;id='.$r->poster_id.'&amp;'._rsid.'">'.$r->alias.'</a>' : $GLOBALS['ANON_NICK'].'' ) : '' ) .'</td>
</tr>';
	}
	if ($uc_sub_forum) {
		$uc_sub_forum = '<tr>
        <th class="wa">Categorie &raquo; Forum</th>
...
'.$uc_sub_forum;
	...
	while ($r = db_rowobj($c)) {
		...
		$uc_sub_topic .= '<tr>
	<td class="RowStyleA"><a href="index.php?t='.d_thread_view.'&amp;th='.$r->id.'&amp;unread=1&amp;'._rsid.'"><img src="theme/default/images/newposts.gif" title="Eerste ongelezen bericht in dit onderwerp weergeven" alt="" /></a>&nbsp;<a class="big" href="index.php?t='.d_thread_view.'&amp;th='.$r->id.'&amp;'._rsid.'">'.$r->subject.'</a> '.$mini_thread_pager.'<br /><div class="ar"><a href="index.php?t=post&amp;th_id='.$r->id.'&amp;'._rsid.'">Beantwoorden</a> || <a href="index.php?t=uc&amp;utid='.$r->id.'&amp;'._rsid.'&amp;SQ='.$GLOBALS['sq'].'">Uitschrijven</a></div></td>
	<td class="RowStyleB ac">'.$r->replies.'</td>
	<td class="RowStyleB ac">'.$r->views.'</td>
	<td class="RowStyleC ar nw"><span class="DateText">'.strftime('%a, %d %B %Y', $r->post_stamp).'</span><br />Door: '.($r->alias ? '<a href="index.php?t=usrinfo&amp;id='.$r->poster_id.'&amp;'._rsid.'">'.$r->alias.'</a>' : $GLOBALS['ANON_NICK'].'' ) .' <a href="index.php?t='.d_thread_view.'&amp;goto='.$r->last_post_id.'&amp;'._rsid.'#msg_'.$r->last_post_id.'"><img src="theme/default/images/goto.gif" title="Naar het laatste bericht in dit onderwerp gaan" alt="" /></a></td>
</tr>';
	}
	if ($uc_sub_topic) {
		$uc_sub_topic = '<tr>
        <th class="wa">Onderwerp</th>
	<th class="nw">Antwoorden</th>
        <th class="nw">keer bekeken</th>
        <th class="nw">Laatste bericht</th>
</tr>
'.$uc_sub_topic;
	}
...
<tr>
	<th colspan="3">Nieuwe privéberichten</th>
</tr>
<?php echo $uc_new_pms; ?>
</table>
<br /><br />
<table cellspacing="1" cellpadding="2" class="ContentTable">
<tr>
	<th colspan="4">Geabonneerde forums met nieuwe berichten</th>
</tr>
...
<table cellspacing="1" cellpadding="2" class="ContentTable">
<tr>
	<th colspan="4">Geabonneerde onderwerpen met nieuwe berichten</th>
</tr>
