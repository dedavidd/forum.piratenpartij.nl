...
* $Id: mmod.php.t 5221 2011-04-22 11:30:48Z naudefj $
*
...
function send_notifications($to, $msg_id, $thr_subject, $poster_login, $id_type, $id, $frm_name, $frm_id)
{
	...
	if ($GLOBALS['FUD_OPT_2'] & 64) {	// NOTIFY_WITH_BODY
		...
		$iemail_unsub = html_entity_decode($id_type == 'thr' ? '[ <a href="index.php?t=rview&th='.$id.'">Abonnement op dit onderwerp opheffen</a> ]' : '[ <a href="index.php?t=rview&frm_id='.$id.'">Abonnement op dit forum opheffen</a> ]');

		$body_email = $boundry .'Content-Type: text/plain; charset='. $CHARSET ."; format=flowed\r\nContent-Transfer-Encoding: 8bit\r\n\r\n" . html_entity_decode(strip_tags($plain_text)) . "\r\n\r\n" . html_entity_decode('Volg de volgende koppeling om deel te nemen in de discussie:') .' '. ''.$GLOBALS['WWW_ROOT'].'index.php?t=rview&'. ($id_type == 'thr' ? 'th' : 'frm_id') .'='. $id ."\r\n".
	...
	if ($id_type == 'thr') {
		...
		if (!isset($body_email)) {
			...
			$body_email = html_entity_decode('Ga naar '.$goto_url['email'].' om ongelezen antwoorden te lezen.\n\nAls u geen verdere waarschuwingen over dit onderwerp wilt ontvangen, volg dan de volgende koppeling: '.$unsub_url['email']);
		}
	} else if ($id_type == 'frm') {
		...
		if (!isset($body_email)) {
			...
			$body_email = html_entity_decode('Volg de volgende koppeling om het onderwerp te bekijken:\n'.$goto_url['email'].'\n\nAls u geen verdere waarschuwingen over nieuwe onderwerpen in dit forum wilt ontvangen, volg dan de volgende koppeling: '.$unsub_url['email']);
		}
	}
...
/* Print number of unread private messages in User Control Panel. */
	...
	if ($del) {
		...
		if (empty($_POST['confirm'])) {
			if ($data[2] != $data[4]) {
				$delete_msg = 'U staat op het punt het bericht <b>'.$data[3].'</b> te <span class="ErrorText">verwijderen</span>';
			} else {
				$delete_msg = 'U staat op het punt het <span class="ErrorText">hele onderwerp</span> <b>'.$data[3].'</b> te <span class="ErrorText">verwijderen</span>';
			}
			?> <!DOCTYPE html>
<html lang="nl" dir="ltr">
<head>
...
<tr>
	<th>Bevestig verwijderen van onderwerp of bericht</th>
</tr>
	...
	<td class="GenText">
		<form method="post" action="index.php?t=mmod">
			...
			<div class="mnavWarnTbl">
				<b>Optioneel</b><br />Geen een reden op voor het verwijderen van het bericht. Dit wordt naar de auteur verzonden via een privébericht.<br />
				<textarea name="del_reason" style="width:98%; height:220px;"></textarea>
				<br />
				<label><input type="checkbox" name="del_inc_body" /> De inhoud van het verwijdere bericht aan het bericht aan de auteur toevoegen.</label>
				<br /><br />
			</div>
			<br />
			<span class="fb"><b>Wilt u doorgaan?</b></font></span><br />
			<input type="submit" class="button" name="YES" value="Ja" />
			&nbsp;&nbsp;&nbsp;
			<input type="submit" class="button" name="NO" value="Nee" />
		</form>
	</td>
</tr>
</table>
...
'.$body;
				...
				send_status_update($data[12], $data[13], $data[14], 'Uw bericht is verwijderd door een beheerder.', $body);
			}
			...
			if ($data[2] == $data[4]) {
				...
				if (strpos($usr->returnto, 'selmsg') === false) {
					if ($FUD_OPT_2 & 32768) {
						header('Location: '.$GLOBALS['WWW_ROOT'].'index.php/f/'. $data[0] .'/'. _rsidl);
					} else {
						header('Location: '.$GLOBALS['WWW_ROOT'].'index.php?t='. t_thread_view .'&'. _rsidl .'&frm_id='. $data[0]);
					}
					exit;
				} else {
					check_return($usr->returnto);
		...
		if (d_thread_view == 'tree') {
			if (!$data[5]) {
				if ($FUD_OPT_2 & 32768) {
					header('Location: '.$GLOBALS['WWW_ROOT'].'index.php/mv/tree/'. $data[1] .'/'. _rsidl);
				} else {
					header('Location: '.$GLOBALS['WWW_ROOT'].'index.php?t=tree&'. _rsidl .'&th='. $data[1]);
				}
			} else {
				if ($FUD_OPT_2 & 32768) {
					header('Location: '.$GLOBALS['WWW_ROOT'].'index.php/mv/tree/'. $data[1] .'/'. $data[5] .'/'. _rsidl);
				} else {
					header('Location: '.$GLOBALS['WWW_ROOT'].'index.php?t=tree&'. _rsidl .'&th='. $data[1] .'&mid='. $data[5]);
				}
			}
		} else {
			...
			if ($FUD_OPT_2 & 32768) {
				header('Location: '.$GLOBALS['WWW_ROOT'].'index.php/mv/msg/'. $data[1] .'/'. $prev_id .'/'. _rsidl);
			} else {
				header('Location: '.$GLOBALS['WWW_ROOT'].'index.php?t=msg&th='. $data[1] .'&'. _rsidl .'&goto='. $prev_id);
			}
		}
		exit;
