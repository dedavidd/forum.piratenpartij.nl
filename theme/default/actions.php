...
function make_perms_query(&$fields, &$join, $fid='')
{
	...
	ses_update_status($usr->sid, 'Activiteiten van andere gebruikers aan het bekijken, net als u.');
	...
	while ($r = db_rowarr($c)) {
		...
		if ($r[3]) {
			...
			if (!$r[10]) {
				...
				$last_post = (!$is_a && !$r[12] && empty($limit[$r[11]])) ? 'U hebt niet de benodigde rechten om dit onderwerp te lezen.' : strftime('%a, %d %B %Y %H:%M', $r[10]).'
<br />
<a href="index.php?t='.d_thread_view.'&goto='.$r[8].'&'._rsid.'#msg_'.$r[8].'">'.$r[9].'</a>';
		...
		if (!$r[2] || ($is_a || !empty($limit[$r[2]]) || $r[13])) {
			...
			$action = 'U hebt niet de benodigde rechten om dit onderwerp te lezen.';
		}
...
<div class="GenText ac">
	[ <a href="index.php?t=actions&rand=<?php echo get_random_value(); ?>&<?php echo _rsid; ?>" rel="nofollow">Lijst verversen</a> ]
</div>
<br />
<table cellspacing="1" cellpadding="2" class="ContentTable">
<tr>
	<th><a href="index.php?t=actions&amp;o=alias&amp;s=<?php echo ($o=='u.alias' && $s=='ASC' ? 'd' : 'a'); ?>&amp;<?php echo _rsid; ?>" class="thLnk" rel="nofollow">Gebruiker</a></th>
	<th>handeling</th>
	<?php echo ($is_a ? '<th><a href="index.php?t=actions&amp;o=ip&amp;s='.($o=='s.time_sec' && $s=='ASC' ? 'd' : 'a' )  .'&amp;'._rsid.'" class="thLnk" rel="nofollow">IP-adres</a></th>' : '' )  .'
	<th><a href="index.php?t=actions&amp;o=time&amp;s='.($o=='s.time_sec' && $s=='ASC' ? 'd' : 'a'); ?>&amp;<?php echo _rsid; ?>" class="thLnk" rel="nofollow">Tijd</a></th>
