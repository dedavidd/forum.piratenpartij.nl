...
/* Print number of unread private messages in User Control Panel. */
	...
	if (isset($_POST['posted']) && _uid && !check_femail_form()) {
		...
		error_dialog('De e-mail is verzonden', 'De e-mail naar uw vriend met e-mailadres '.htmlspecialchars($_POST['femail']).' over het onderwerp '.$data->subject.' is verzonden.');
	...
	<td class="GenText nw">Uw naam:</td>
	<td width="100%">'.$usr->alias.'</td>
</tr>
<tr class="'.alt_var('page_alt','RowStyleA','RowStyleB').'">
	<td class="GenText nw">Uw e-mailadres:</td>
	<td width="100%">'.$usr->email.'</td>
</tr>
<tr class="'.alt_var('page_alt','RowStyleA','RowStyleB').'">
	<td class="GenText nw">Naam van uw vriend:</td>
	<td width="100%"><input type="text" name="fname" value="'.(isset($_POST['fname']) ? htmlspecialchars($_POST['fname']).'' : '' )  .'" /></td>
</tr>
<tr class="'.alt_var('page_alt','RowStyleA','RowStyleB').'">
	<td class="GenText nw vt SmallText">E-mailadres van uw vriend:<br /><i>vereist</i></td>
	<td valign="top"><input type="text" name="femail" value="'.(isset($_POST['femail']) ? htmlspecialchars($_POST['femail']).'' : '' )  .'">'.get_err('femail').'</td>
</tr>
<tr class="'.alt_var('page_alt','RowStyleA','RowStyleB').'">
	<td class="GenText nw vt SmallText">Onderwerp:<br /><i>vereist</i></td>
	<td valign="top" class="nw"><input type="text" spellcheck="true" name="subj" value="'.(isset($_POST['subject']) ? htmlspecialchars($_POST['subject']).'' : $data->subject.'' )  .'" size="60" />'.get_err('subj').'</td>
</tr>
<tr class="'.alt_var('page_alt','RowStyleA','RowStyleB').'">
	<td class="GenText nw vt">Bericht:<span class="SmallText"><br /><i>vereist</i></span></td>
	<td valign="top" class="nw"><textarea name="body" rows="19" cols="78" wrap="PHYSICAL">'.(isset($_POST['body']) ? htmlspecialchars($_POST['body']).'' : 'Hallo.\n\nEr is een interessant onderwerp over "'.$data->subject.'" op het forum '.$GLOBALS['FORUM_TITLE'].' dat u misschien wel wilt lezen. U kunt dit onderwerp bekijken via de volgende koppeling:\n  '.$GLOBALS['WWW_ROOT'].'index.php?t=rview&amp;th='.$data->id.'&amp;rid='._uid.'\n\n Groet,\n\n '.$usr->alias.'\n' ) .'</textarea>'.get_err('body').'</td>
</tr>
<tr class="'.alt_var('page_alt','RowStyleA','RowStyleB').'">
	<td class="GenText ar" colspan="2"><input type="submit" class="button" name="submit" value="E-mail verzenden" /></td>
</tr>' : '<tr class="'.alt_var('page_alt','RowStyleA','RowStyleB').'">
	<td class="GenText ac SmallText">Dit bericht kopiëren naar een e-mailprogramma van uw keuze om het naar een contactpersoon te verzenden.</td>
</tr>
<tr class="'.alt_var('page_alt','RowStyleA','RowStyleB').'">
	<td class="GenText"><textarea name="body" rows="19" cols="78">'.(isset($_POST['body']) ? htmlspecialchars($_POST['body']).'' : 'Hallo.\n\nEr is een interessant onderwerp over "'.$data->subject.'" op het forum '.$GLOBALS['FORUM_TITLE'].' dat u misschien wel wilt lezen. U kunt dit onderwerp bekijken via de volgende koppeling:\n  '.$GLOBALS['WWW_ROOT'].'index.php?t=rview&amp;th='.$data->id.'&amp;rid='._uid.'\n\n Groet,\n\n '.$usr->alias.'\n' ) .'</textarea></td>
</tr>
<tr class="'.alt_var('page_alt','RowStyleA','RowStyleB').'">
	<td class="GenText ar"><input type="submit" class="button" name="done" value="Afgerond" /></td>
</tr>';
...
<div class="ctb">
<?php echo (is_post_error() ? '<h4 class="ac ErrorText">Er is een fout opgetreden</h4>' : ''); ?>
<form action="index.php?t=remail" id="remail" method="post">
<input type="hidden" name="posted" value="1" />
<table cellspacing="1" cellpadding="2" class="MiniTable">
<tr>
	<th colspan="2">Dit onderwerp naar een vriend e-mailen</th>
</tr>
