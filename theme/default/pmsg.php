...
* $Id: pmsg.php.t 5508 2012-06-08 20:58:38Z naudefj $
*
...
<tr>
	...
	if (isset($_POST['sel']) || isset($_GET['sel'])) {
		...
		if (!$move_to && !isset($_POST['old_folder_id'])) {
			$_POST['old_folder_id'] = q_singleval('SELECT fldr FROM fud30_pmsg WHERE id='. $sel[0]);
		}
	...
	while ($obj = db_rowobj($c)) {
		...
		if ($FUD_OPT_2 & 32 && (!($obj->users_opt & 32768) || $is_a)) {
			...
			if (($obj->time_sec + $LOGEDIN_TIMEOUT * 60) > __request_timestamp__) {
				$online_indicator = '<img src="theme/default/images/online'.img_ext.'" alt="'.$obj->login.' is op dit moment aanwezig" title="'.$obj->login.' is op dit moment aanwezig" />&nbsp;';
			} else {
				$online_indicator = '<img src="theme/default/images/offline'.img_ext.'" alt="'.$obj->login.' is op dit moment afwezig" title="'.$obj->login.' is op dit moment afwezig" />&nbsp;';
			}
		} else {
			$online_indicator = '';
		}
		...
		if ($obj->pmsg_opt & 64) {
			$msg_type ='<span class="SmallText">(beantwoord)</span>&nbsp;';
		} else if ($obj->pmsg_opt & 32) {
			$msg_type = '';
		} else {
			$msg_type ='<span class="SmallText">(doorgestuurd)</span>&nbsp;';
		}

		$private_msg_entry .= '<tr class="RowStyleB">
	<td class="ac GenText"><input type="checkbox" name="sel[]" value="'.$obj->id.'" /></td>
	<td>'.($obj->read_stamp ? '<img src="theme/default/images/pmsg_read'.img_ext.'" alt="Gelezen privébericht" title="Gelezen privébericht" />' : '<img src="theme/default/images/pmsg_unread'.img_ext.'" alt="Ongelezen privébericht" title="Ongelezen privébericht" />' ) .'</td>
	<td width="100%" class="GenText">'.$msg_type.'<a href="'.$goto.'">'.$obj->subject.'</a>'.(($obj->pmsg_opt & 4 && $obj->pmsg_opt & 16 && $obj->duser_id == _uid && $obj->ouser_id != _uid) ? '<span class="SmallText">&nbsp;&nbsp;[<a href="index.php?t=pmsg_view&amp;'._rsid.'&amp;dr=1&amp;id='.$obj->id.'" title="Geen leesbevestiging verzenden">geen ontvangstbevestiging verzenden</a>]</span>' : '' ) .'</td>
	...
	if (!$private_msg_entry) {
		$private_msg_entry = '<tr class="RowStyleC"><td colspan="6" class="ac">Er zijn geen berichten in deze map</td></tr>';
		$private_tools = '';
	} else {
		if ($folder_id == 5) {
			$btn_action = 'Herstellen naar:';
			$btn_del_name = 'btn_trash';
			$btn_del_title = 'Lege prullenbak';
		} else {
			$btn_action = 'Verplaatsen naar:';
			$btn_del_name = 'btn_delete';
			$btn_del_title = 'Verwijderen';
		}
		$tmp = $folders;
		unset($tmp[$folder_id]);
		$moveto_list = tmpl_draw_select_opt(implode("\n", array_keys($tmp)), implode("\n", $tmp), 2);
		$private_tools = '<tr class="RowStyleB">
	<td class="SmallText al" colspan="3"><a href="index.php?t=pmsg_dump&amp;'._rsid.'">Berichtendump downloaden</a></td>
	<td colspan="3" class="SmallText ar">
		...
		&nbsp;&nbsp;&nbsp;<input type="submit" class="button" name="'.$btn_del_name.'" value="'.$btn_del_title.'" />'.($FUD_OPT_2 & 2097152 ? '&nbsp;&nbsp;&nbsp;<input type="submit" class="button" name="btn_pdf" value="PDF-archief maken" />' : '' )  .'
	</td>
</tr>';
	}
...
<tr>
	<td colspan="3" class="RowStyleA wa nw SmallText">Uw map voor privéberichten is voor <?php echo $percent_full; ?>% gevuld.</td>
</tr>
<tr>
...
<tr class="RowStyleB">
	<td class="SmallText al" colspan="3"><?php echo ($private_tools ? '<a href="index.php?t=pmsg_dump&amp;'._rsid.'">Berichtendump downloaden</a>' : '&nbsp;' )  .'</td>
	<td colspan="3" class="SmallText ar nw">
		Map:
		<select name="folder_id" onchange="document.forms[\'priv_frm\'].submit();">
		'.$select_options_cur_folder.'
		</select> <input type="submit" class="button" name="sel_folder" value="OK" />
	</td>
</tr>
<tr>
	...
	<th>&nbsp;</th>
	<th class="wa"><a href="index.php?t=pmsg&amp;folder_id='.$folder_id.'&amp;'._rsid.'&amp;o=subject&amp;s='.($o=='p.subject' && $s=='ASC' ? 'd' : 'a'); ?>" class="thLnk">Onderwerp</a></th>
	<th class="ac"><a href="index.php?t=pmsg&amp;folder_id=<?php echo $folder_id; ?>&amp;<?php echo _rsid; ?>&amp;o=<?php echo ($folder_id == 3 ? 'alias2' : 'alias' )  .'&amp;s='.(($o=='u.alias' || $o=='u2.alias') && $s=='ASC' ? 'd' : 'a' )  .'" class="thLnk">'.($folder_id == 3 ? 'Ontvanger' : 'Auteur'); ?></a></th>
	<th class="ac"><a href="index.php?t=pmsg&amp;folder_id=<?php echo $folder_id; ?>&amp;<?php echo _rsid; ?>&amp;o=post_stamp&amp;s=<?php echo ($o=='p.post_stamp' && $s=='ASC' ? 'd' : 'a'); ?>" class="thLnk">Tijd</a></th>
