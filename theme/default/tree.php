...
* $Id: tree.php.t 5488 2012-05-16 15:10:50Z naudefj $
*
...
' : '' )  ;
	...
	if (_uid) {
		...
		$subscribe_status = $frm->subscribed ? '| <a href="index.php?t=tree&amp;th='.$th.'&amp;notify='.$usr->id.'&amp;'._rsid.'&amp;opt=off&amp;mid='.$mid.'&amp;SQ='.$GLOBALS['sq'].'" title="Bij nieuwe berichten in dit onderwerp geen waarschuwingen meer verzenden">Abonnenment op dit onderwerp opheffen</a>&nbsp;' : '| <a href="index.php?t=tree&amp;th='.$th.'&amp;notify='.$usr->id.'&amp;'._rsid.'&amp;opt=on&amp;mid='.$mid.'&amp;SQ='.$GLOBALS['sq'].'" title="Een bericht ontvangen aks er nieuwe berichten in dit onderwerp zijn">Op dit onderwerp abonneren</a>&nbsp;';
		$bookmark_status  = $frm->bookmarked ? '| <a href="index.php?t=tree&amp;th='.$th.'&amp;bookmark='.$usr->id.'&amp;'._rsid.'&amp;opt=off&amp;mid='.$mid.'&amp;SQ='.$GLOBALS['sq'].'" title="Bladwijzer voor dit onderwerp verwijderen">Bladwijzer verwijderen</a>&nbsp;' : '| <a href="index.php?t=tree&amp;th='.$th.'&amp;bookmark='.$usr->id.'&amp;'._rsid.'&amp;opt=on&amp;mid='.$mid.'&amp;SQ='.$GLOBALS['sq'].'" title="Dit onderwerp aan uw bladwijzers toevoegen">Bladwijzer voor onderwerp toevoegen</a>&nbsp;';
	} else {
	...
	ses_update_status($usr->sid, 'Onderwerp <a href="index.php?t=tree&amp;th='.$frm->id.'&amp;'._rsid.'">'.$frm->subject.'</a> aan het bekijken in boomstructuur', $frm->id);
...
function draw_forum_path($cid, $fn='', $fid=0, $tn='')
{
	...
	if ($perms & 4096) {
		$lock_thread = !($frm->thread_opt & 1) ? '<a href="index.php?t=mmod&amp;'._rsid.'&amp;th='.$th.'&amp;lock=1&amp;SQ='.$GLOBALS['sq'].'">Onderwerp sluiten</a>&nbsp;|&nbsp;' : '<a href="index.php?t=mmod&amp;'._rsid.'&amp;th='.$th.'&amp;unlock=1&amp;SQ='.$GLOBALS['sq'].'">Onderwerp ontgrendelen</a>&nbsp;|&nbsp;';
	} else {
		$lock_thread = '';
	}
	...
	<td>'.((_uid && $cur->post_stamp > $usr->last_read && $cur->post_stamp > $frm->last_view) ? '<img id="msgIm'.$cur->id.'" src="theme/default/images/unread'.img_ext.'" alt="Ongelezen berichten" title="Ongelezen berichten" />' : '<img src="theme/default/images/read'.img_ext.'" alt="Berichten lezen" title="Berichten lezen" />' ) .'</td>
	...
	<td class="Gentext nw wa vt" style="padding-left: '.(15 * ($lev - 1)).'px">
		...
		<div class="TopBy">Door: '.($cur->poster_id ? '<a href="index.php?t=usrinfo&amp;id='.$cur->poster_id.'&amp;'._rsid.'">'.$cur->alias.'</a>' : $GLOBALS['ANON_NICK'].'' ) .' op '.strftime('%a, %d %B %Y %H:%M', $cur->post_stamp).'</div>
	</td>
</tr>';
...
<b>::</b> <a href="index.php?t=mnav&amp;<?php echo _rsid; ?>" rel="nofollow">Navigatie</a>
		...
		<?php echo ($perms & 8192 ? ($frm->replies ? '<a href="index.php?t=movemsg&amp;th='.$th.'">Berichten verplaatsen</a>&nbsp;|&nbsp;' : '' )  .'<a href="javascript://" onclick="window_open(\''.$GLOBALS['WWW_ROOT'].'index.php?t=mvthread&amp;'._rsid.'&amp;th='.$th.'\', \'th_move\', 300, 400);">Verplaatsen</a>&nbsp;|&nbsp;' : '' ) .$lock_thread.'
	...
	<td class="vb ar">
		<a href="index.php?t=msg&amp;th=<?php echo $th; ?>&amp;<?php echo _rsid; ?>#msg_<?php echo $mid; ?>"><img alt="Terug naar de platte weergave" title="Terug naar de platte weergave" src="theme/default/images/flat_view.gif" /></a>&nbsp;<a href="index.php?t=post&amp;frm_id=<?php echo $frm->forum_id; ?>&amp;<?php echo _rsid; ?>"><img alt="Nieuw onderwerp starten" src="theme/default/images/new_thread.gif" /></a><?php echo ((!($frm->thread_opt & 1) || $perms & 4096) ? '&nbsp;<a href="index.php?t=post&amp;th_id='.$th.'&amp;reply_to='.$mid.'&amp;'._rsid.'"><img alt="Antwoorden" src="theme/default/images/post_reply.gif" /></a>' : ''); ?>
	</td>
</tr>
</table>
...
<div class="ac GenText">[<a href="javascript://" onclick="chng_focus('tree_view');" title="Berichtenlijst bekijken">Berichtenlijst</a>]</div>
<a name="tree_view">&nbsp;</a>
