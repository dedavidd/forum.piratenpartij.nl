...
function error_check()
{
	...
	if ($usr_d || isset($_POST['login']) && !error_check()) {
		...
		if (!$usr_d && !($usr_d = db_sab('SELECT last_login, id, passwd, salt, login, email, users_opt, ban_expiry FROM fud30_users WHERE login='. _esc($_POST['login'])))) {
			...
			login_php_set_err('login', 'Dit forum staat iedere '.$MIN_TIME_BETWEEN_LOGIN.' seconden een aanmeldpoging toe. Wacht '.($usr_d->last_login + $MIN_TIME_BETWEEN_LOGIN - __request_timestamp__).' voor uw volgende poging.');
...
<tr>
	...
	<ol>
		<li class="GenText">Uw cookie is verlopen. Meld u opnieuw aan om uw cookie te vernieuwen.</li>
