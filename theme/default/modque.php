...
* $Id: modque.php.t 5460 2012-04-23 06:35:36Z naudefj $
*
...
function reverse_nl2br($data)
{
	...
	if ($appr || $del) {
		if (!q_singleval('SELECT CASE WHEN ('. q_bitand($usr->users_opt, 1048576) .' = 0) THEN mm.id ELSE 1 END FROM fud30_msg m INNER JOIN fud30_thread t ON m.thread_id=t.id LEFT JOIN fud30_mod mm ON t.forum_id=mm.forum_id AND mm.user_id='. _uid .' WHERE m.id='. ($appr ? $appr : $del))) {
			if (db_affected()) {
				std_error('perms');
			} else {
				$del = $appr = 0;
...
// TODO: Create a new REJECTMSG action?
	...
	<td>
		<b>Categorie</b> '.$obj->cat_name.' :: 
		<b>Forum</b> '.$obj->frm_name.' 
		[<a href="index.php?t=modque&amp;appr='.$obj->id.'&amp;'._rsid.'&amp;SQ='.$GLOBALS['sq'].'">Goedkeuren</a>]&nbsp;&nbsp;
		[<a href="index.php?t=modque&amp;del='.$obj->id.'&amp;'._rsid.'&amp;SQ='.$GLOBALS['sq'].'">Verwijderen</a>]
	</td>
</tr>
'.tmpl_drawmsg($obj, $usr, $perms, false, $m_num, null);
	...
	if (!$modque_message) {
		$modque_message = '<tr class="RowStyleA"><td>Er zijn geen berichten die nog goedgekeurd moeten worden</td></tr>';
	}
...
<tr>
	<th>Berichten die nog goedgekeurd moeten worden</th>
</tr>
